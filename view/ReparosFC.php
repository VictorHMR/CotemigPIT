  ...
  <table class="Finalizado">
    ...
    <?php foreach($pedidosF as $objF){ ?> 
      <tr>
        ...
        <?php  if($objF->status != "Aguardando Avaliação" && $objF->status != "Avaliado Pelo Tecnico"  ): ?> <td></td> <?php endif; ?>
        <?php  if($objF->status == "Aguardando Avaliação" || $objF->status == "Avaliado Pelo Tecnico" ): ?> <td><button type="button" class="btnAvaliar" data-toggle="modal" data-target="#modalAva" data-user="<?=$objF->usuario?>" data-id="<?=$objF->id?>"><i class="bi bi-award-fill"></i></button> </td> <?php endif; ?>
      ...
      <div class="modal-body">
        ...
        <div class="tempo" >
         <h4>Avalie o Tecnico</h4>
            ...
            <input type="radio" id="ava5" name="fb" value="5" onclick="nota()"/>
            <br><br>
...
var Nota;
 ...
 $('#modalAva').on('shown.bs.modal', function(event) {
    ...
    $('#modal-btn-avaliar').on('click', function(event){
      ...
      $("#modal-btn-avaliar").attr('href', './?ava=avaliar&id='+id+'&cliente='+Usuario+'&nota='+Nota+'&coment='+Coment)
    });
});
